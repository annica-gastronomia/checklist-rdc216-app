...
function normalizeAppccProcesso(p){return {id:p.id||genId('processo'),nome:p.nome||'',tipo:p.tipo||'restaurante',fluxo:Array.isArray(p.fluxo)?p.fluxo.map(normalizeAppccFluxoItem):[],analises:Array.isArray(p.analises)?p.analises.map(normalizeAppccAnalise):[]};}
