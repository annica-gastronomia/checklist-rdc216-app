...
{match:norm=>norm.includes('recebiment')&&/peixe|pescad|sashimi|sushi|frutos do mar|marisco|camarao|salmao|atum/.test(norm),tipoPerigo:'Biológico',perigo:'Recebimento de pescado fora da temperatura exigida (2 a 3°C para pescado refrigerado, ou 0 a 5°C para preparações com pescado cru, conforme Art. 30 da Portaria CVS 3/2026), fora do prazo de validade ou das condições higiênico-sanitárias, favorecendo a proliferação de microrganismos patogênicos e a presença de parasitas (ex.: Anisakis).',controle:'Conferir e registrar a temperatura do pescado no recebimento (2 a 3°C, ou conforme recomendação do frigorífico produtor — Art. 30 da Portaria CVS 3/2026), validade, integridade da embalagem, aspecto sensorial (odor, cor, textura) e procedência do fornecedor; recusar o lote se algum critério não for atendido.'},
{match:norm=>norm.includes('recebiment')&&/carne|bovin|suin|frango|ave\b|aves\b|\bboi\b|linguic|embutid/.test(norm),tipoPerigo:'Biológico',perigo:'Recebimento de carne fora da temperatura exigida (4 a 7°C, conforme Art. 30 da Portaria CVS 3/2026), fora do prazo de validade ou das condições higiênico-sanitárias, favorecendo a proliferação de microrganismos patogênicos.',controle:'Conferir e registrar a temperatura da carne no recebimento (4 a 7°C, ou conforme recomendação do frigorífico produtor — Art. 30 da Portaria CVS 3/2026), validade, integridade da embalagem e procedência do fornecedor; recusar o lote se algum critério não for atendido.'},
...
function appccResolveHazardSuggestion(tipoPerigo,etapaTexto){const norm=appccNormalizeText(etapaTexto);const override=APPCC_ETAPA_HAZARD_OVERRIDES.find(row=>row.tipoPerigo===tipoPerigo&&(row.match?row.match(norm):row.keywords.some(k=>norm.includes(k))));if(override)return {perigo:override.perigo,controle:override.controle};return APPCC_HAZARD_SUGGESTIONS[tipoPerigo]||null;}
const APPCC_LIMITE_HINTS=[
{match:norm=>norm.includes('recebiment')&&/peixe|pescad|sashimi|sushi|frutos do mar|marisco|camarao|salmao|atum/.test(norm),texto:'Critério objetivo que separa aceitável de não aceitável. Ex.: pescado refrigerado entre 2°C e 3°C no recebimento (Art. 30 da Portaria CVS 3/2026).'},
{match:norm=>norm.includes('recebiment')&&/carne|bovin|suin|frango|ave\b|aves\b|\bboi\b|linguic|embutid/.test(norm),texto:'Critério objetivo que separa aceitável de não aceitável. Ex.: carne entre 4°C e 7°C no recebimento (Art. 30 da Portaria CVS 3/2026).'},
...
{match:norm=>/higien|lavagem|sanitiz/.test(norm),texto:'Critério objetivo que separa aceitável de não aceitável. Ex.: tempo de contato do desinfetante conforme instrução do fabricante.'}
];
function appccSuggestLimiteHint(etapaTexto){const norm=appccNormalizeText(etapaTexto);const hint=APPCC_LIMITE_HINTS.find(row=>row.match(norm));return hint?hint.texto:'Critério objetivo que separa aceitável de não aceitável. Ex.: cocção acima de 75°C.';}
